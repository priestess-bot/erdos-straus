...
class H4CleanQMacroVerifierTests(unittest.TestCase):
    def test_existing_control_fixtures_pass_e1_e5(self):
        ...
        for r in results:
            ...
            self.assertEqual(r["verification_scope"], "local_arithmetic_control_only")
            self.assertEqual(r["selector_status"], "control_only")
            self.assertFalse(r["phase_local_macro_eligible"])
            self.assertFalse(r["recursive_edge_eligible"])
    ...
    def test_target_type_label_is_not_inherited(self):
        ...
        self.assertFalse(receipt["target"]["selector_consumable"])
        self.assertFalse(receipt["target"]["inherited_type_label"])
        self.assertEqual(receipt["target"]["mandatory_next_step"], "normalize_target_state")
        self.assertFalse(receipt["typed_reclassification"]["inherited_label"])

    def test_lazy_dispatch_fields_do_not_enter_state_identity(self):
        receipt = h4.verify_h4_macro(h4.make_control_input(**h4.CONTROL_FIXTURES[0]))
        self.assertNotIn("dispatch_status", receipt["target_identity"])
        self.assertNotIn("inherited_type_label", receipt["target_identity"])
        self.assertNotIn("capacity", receipt["target_identity"])

    def test_verified_premises_enable_a_phase_local_macro_only(self):
        inp = h4.make_control_input(**h4.CONTROL_FIXTURES[0])
        verified = h4.H4Input(
            p=inp.p, r4=inp.r4, k4=inp.k4, m4=inp.m4, c4=inp.c4,
            persistent_parent=inp.persistent_parent,
            priority_prefix={
                **inp.priority_prefix,
                "validation_status": h4.PRIORITY_PREMISE_ACTUAL,
            },
            upstream_h4={
                **inp.upstream_h4,
                "validation_status": h4.UPSTREAM_PROVENANCE_ACTUAL,
            },
        )
        receipt = h4.verify_h4_macro(verified)
        self.assertEqual(receipt["selector_status"], "candidate_transition")
        self.assertTrue(receipt["phase_local_macro_eligible"])
        self.assertFalse(receipt["recursive_edge_eligible"])

    def test_wrong_upstream_claim_is_rejected(self):
        inp = h4.make_control_input(**h4.CONTROL_FIXTURES[0])
        bad = h4.H4Input(
            p=inp.p, r4=inp.r4, k4=inp.k4, m4=inp.m4, c4=inp.c4,
            persistent_parent=inp.persistent_parent,
            priority_prefix=inp.priority_prefix,
            upstream_h4={**inp.upstream_h4, "claim_id": "wrong-claim"},
        )
        with self.assertRaises(h4.VerificationError):
            h4.verify_h4_macro(bad)
