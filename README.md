...
checkpoint contraction，并以 common state classifier 入队 final target。\(p=73\) 覆盖
overflow/`LOCAL_DROP`，\(p=601\) 覆盖 ABSORB/`PHASE_DROP`；两者的 final state 都被
显式验证为 `DEAD_END`，所以这只是可重放的 producer slice，并不宣称 post-G re-entry
已经解决。见
