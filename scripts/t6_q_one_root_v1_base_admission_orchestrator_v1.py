#!/usr/bin/env python3
"""Exact-HEAD assembly of one q=1 G V1 base-admission receipt.

This controlled-loader boundary is deliberately narrow.  It accepts only an
exact repository location, a full requested HEAD, raw q=1 G source integers,
and a V3 production terminal receipt.  It derives both V5 grants and the
needed V4 owner/scope grants from exact-HEAD registry resolution; callers
cannot supply a grant, an owner receipt, a scope receipt, a V4 E1 receipt, or
a queue token.

The output is a serialized ``Q1_ROOT_V1_BASE_ADMISSION_RECEIPT_V1``.  It is a
base admission for one parentless ``ROOT_INITIALIZER_OUTPUT`` occurrence, not
a queue operation, successor authorization, producer continuation, E1--E5,
T5, or global result.
"""
...
def _canonical(value: Any) -> bytes:
    try:
        ...
        raise OrchestratorError(OrchestratorRejectCode.MALFORMED_INPUT, str(exc)) from exc
...
def _fresh_module(
    ...
    try:
        ...
        raise OrchestratorError(
            OrchestratorRejectCode.MODULE_BINDING_ERROR, f"fresh {path}: {exc}"
        ) from exc
    finally:
        if previous is None:
...
def _fresh_v1_bundle(
    ...
    """Fresh-exec V1 modules while pinning their ordinary local imports.

    Materializer and admission verifier use ordinary imports for the frozen V1
    state contract, root envelope, and adapter.  Temporarily replacing those
    exact canonical module names prevents a stale preloaded module from being
    selected; their globals retain the exact modules after the slots restore.
    """
...
def _artifact_map(resolved: Mapping[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    if type(artifacts) is not list:
        _reject(OrchestratorRejectCode.REGISTRY_ERROR, "resolved artifacts are missing")
    result: dict[str, dict[str, Any]] = {}
    for artifact in artifacts:
        if type(artifact) is not dict or type(artifact.get("artifact_id")) is not str:
            _reject(OrchestratorRejectCode.REGISTRY_ERROR, "malformed resolved artifact")
        artifact_id = artifact["artifact_id"]
        if artifact_id in result:
            _reject(OrchestratorRejectCode.REGISTRY_ERROR, f"duplicate artifact {artifact_id}")
...
def _check_artifact(
    ...
    if artifact is None or entry is None:
        _reject(OrchestratorRejectCode.REGISTRY_ERROR, f"missing artifact pin {artifact_id}")
    if (
        ...
        _reject(OrchestratorRejectCode.MODULE_BINDING_ERROR, f"artifact pin mismatch {artifact_id}")


def _v5_grants(resolved: Mapping[str, Any]) -> tuple[dict[str, dict[str, Any]], dict[str, dict[str, Any]]]:
    if type(resolved) is not dict:
        _reject(OrchestratorRejectCode.REGISTRY_ERROR, "V5 resolver output is not an exact dict")
    if (
        ...
        _reject(OrchestratorRejectCode.REGISTRY_ERROR, "V5 policy boundary changed")
    if (
        ...
        _reject(OrchestratorRejectCode.REGISTRY_ERROR, "V5 role or mutation counts changed")
    ...
    if type(raw_grants) is not list or len(raw_grants) != len(expected):
        _reject(OrchestratorRejectCode.ROLE_GRANT_ERROR, "V5 grant cardinality changed")
    ...
    for item in raw_grants:
        if type(item) is not dict or item.get("role") not in expected:
            _reject(OrchestratorRejectCode.ROLE_GRANT_ERROR, "V5 grant role changed")
        role = item["role"]
        if role in grants:
            _reject(OrchestratorRejectCode.ROLE_GRANT_ERROR, f"duplicate V5 role {role}")
        ...
        if artifact is None or type(wire) is not dict:
            _reject(OrchestratorRejectCode.ROLE_GRANT_ERROR, f"missing V5 grant wire {role}")
        if (
            set(wire) != required
            ...
            _reject(OrchestratorRejectCode.ROLE_GRANT_ERROR, f"V5 grant wire changed {role}")
        grants[role] = _json_copy(wire)
    if set(grants) != set(expected):
        _reject(OrchestratorRejectCode.ROLE_GRANT_ERROR, "V5 grants incomplete")
    ...
    if (
        ...
        _reject(OrchestratorRejectCode.REGISTRY_ERROR, "V5 role manifest changed")
...
def _v4_grants(resolved: Mapping[str, Any]) -> dict[str, dict[str, Any]]:
    if type(resolved) is not dict or resolved.get("status") != V4_STATUS:
        _reject(OrchestratorRejectCode.REGISTRY_ERROR, "V4 status changed")
    if resolved.get("authorized_branches") != [] or resolved.get("new_role_grant_count") != 3:
        _reject(OrchestratorRejectCode.REGISTRY_ERROR, "V4 role boundary changed")
    expected = {
        OWNER_ROLE: (
            OWNER_GRANT_ID,
            OWNER_ID,
    ...
    if type(raw_grants) is not list:
        _reject(OrchestratorRejectCode.REGISTRY_ERROR, "V4 grants missing")
    for item in raw_grants:
        if type(item) is not dict or item.get("role") not in expected:
            continue
        role = item["role"]
        if role in result:
            _reject(OrchestratorRejectCode.ROLE_GRANT_ERROR, f"duplicate V4 role {role}")
        grant_id, artifact_id, path, symbols, capabilities = expected[role]
        wire = item.get("grant_wire")
        artifact = artifacts.get(artifact_id)
        if artifact is None or type(wire) is not dict:
            _reject(OrchestratorRejectCode.ROLE_GRANT_ERROR, f"missing V4 grant {role}")
        if (
            set(wire) != required
            or wire.get("grant_id") != grant_id
            or wire.get("role") != role
            ...
            _reject(OrchestratorRejectCode.ROLE_GRANT_ERROR, f"V4 grant wire changed {role}")
        result[role] = _json_copy(wire)
    if set(result) != set(expected):
        _reject(OrchestratorRejectCode.ROLE_GRANT_ERROR, "V4 owner/scope grants incomplete")
    return result
...
def _verify_v3_result(result: Any, *, head: str, receipt: Mapping[str, Any]) -> None:
    if getattr(result, "status", None) != "PRODUCTION_Q1_TERMINAL_RECEIPT_VERIFIED":
        _reject(OrchestratorRejectCode.V3_RECEIPT_ERROR, "V3 production receipt was not verified")
    if (
        getattr(result, "receipt_type", None) != V3_MISS_TYPE
        or getattr(result, "outcome", None) != V3_MISS_OUTCOME
    ):
        _reject(OrchestratorRejectCode.TERMINAL_SOURCE_NOT_MISS, "only a V3 prefix MISS may enter V5")
    if (
        receipt.get("head_sha") != head
        or receipt.get("receipt_type") != V3_MISS_TYPE
        or receipt.get("outcome") != V3_MISS_OUTCOME
    ):
        _reject(OrchestratorRejectCode.TERMINAL_SOURCE_NOT_MISS, "V3 MISS HEAD/outcome changed")
...
def _check_v4_evidence(owner: Mapping[str, Any], scope: Mapping[str, Any]) -> None:
    ...
    for label, receipt, expected in (("owner", owner, owner_expected), ("scope", scope, scope_expected)):
        if type(receipt) is not dict:
            _reject(OrchestratorRejectCode.ROLE_ERROR, f"V4 {label} serializer returned non-dict")
        for field, wanted in expected.items():
            if type(receipt.get(field)) is not bool or receipt.get(field) is not wanted:
                _reject(OrchestratorRejectCode.AUTHORITY_ERROR, f"V4 {label}.{field} changed")
...
def _check_final_authority(receipt: Mapping[str, Any]) -> None:
    if type(receipt) is not dict:
        _reject(OrchestratorRejectCode.ROLE_ERROR, "admission serializer returned a non-dict")
    required = {
    ...
    for field, expected in required.items():
        ...
        if type(expected) is bool:
            if type(value) is not bool or value is not expected:
                _reject(OrchestratorRejectCode.AUTHORITY_ERROR, f"receipt.{field} changed")
        elif value != expected:
            _reject(OrchestratorRejectCode.AUTHORITY_ERROR, f"receipt.{field} changed")
    state = receipt.get("v1_state")
    if type(state) is not dict or state.get("queue_gate") != "ROOT_INITIALIZER_OUTPUT":
        _reject(OrchestratorRejectCode.SOURCE_ERROR, "receipt no longer describes a V1 root initializer")
    ...
    if (
        ...
        _reject(OrchestratorRejectCode.AUTHORITY_ERROR, "V4 E1/candidate data entered V1 state semantics")


def _recheck_paths(
    root: Path, entries: Mapping[str, tuple[str, str, str]]
) -> None:
    for path in (
        ORCHESTRATOR_PATH,
        V5_RESOLVER_PATH,
...
def assemble_q_one_root_v1_base_admission_v1(
    *,
    root: Path,
    requested_head: str,
    ...
    """Build the one narrow V5 V1-base-admission receipt for a V3 prefix MISS."""
    ...
    expected_path = (repository / ORCHESTRATOR_PATH).resolve()
    if (
    ...
    v5_module = _fresh_module(repository, entries, V5_RESOLVER_PATH, f"_t6_v5_orchestrator_{head}")
    try:
        ...
        _reject(OrchestratorRejectCode.REGISTRY_ERROR, f"V5 resolver rejected HEAD: {exc}")
    if resolved_v5.get("head_sha") != head or resolved_v5.get("head_tree_sha") != tree:
        _reject(OrchestratorRejectCode.REGISTRY_ERROR, "V5 resolver returned another HEAD")
    ...
    v4_module = _fresh_module(repository, entries, V4_RESOLVER_PATH, f"_t6_v4_for_v5_{head}")
    try:
        with _sanitized_git_environment():
    ...
    if (
        ...
        _reject(OrchestratorRejectCode.REGISTRY_ERROR, "V5/V4 cross-registry binding changed")
    grants_v4 = _v4_grants(resolved_v4)

    v3_module = _fresh_module(repository, entries, V3_VERIFIER_PATH, f"_t6_v3_for_v5_{head}")
    try:
        with _sanitized_git_environment():
    ...
    modules = _fresh_v1_bundle(repository, entries, head, "orchestrator")
    ...
    try:
        ...
        _reject(OrchestratorRejectCode.SOURCE_ERROR, f"root initializer rejected raw source: {exc}")
    actualness = terminal.get("root_actualness")
    if type(actualness) is not dict or actualness.get("head_sha") != head:
        _reject(OrchestratorRejectCode.SOURCE_ERROR, "V3 receipt lacks same-HEAD actualness")
    actualness = _json_copy(actualness, path="production_miss_receipt.root_actualness")

    owner_module = _fresh_module(repository, entries, OWNER_PATH, f"_t6_v4_owner_for_v5_{head}")
    scope_module = _fresh_module(repository, entries, SCOPE_PATH, f"_t6_v4_scope_for_v5_{head}")
    try:
        ...
        _reject(OrchestratorRejectCode.ROLE_ERROR, f"V4 owner/scope evidence rejected source: {exc}")
    ...
    try:
        ...
        result = _call(admission, ADMISSION_SERIALIZER)(admission_obj)
    except Exception as exc:
        _reject(OrchestratorRejectCode.ROLE_ERROR, f"V5 base-admission chain rejected source: {exc}")
    _check_final_authority(result)
    if (
        result.get("source_state_id") != source_state.get("state_id")
        or result.get("v4_owner_receipt_id") != owner_wire.get("receipt_id")
        or result.get("v4_scope_receipt_id") != scope_wire.get("receipt_id")
    ):
        _reject(OrchestratorRejectCode.SOURCE_ERROR, "base-admission result changed its source evidence")
...
__all__ = [
    ...
    "assemble_q_one_root_v1_base_admission_v1",
]
