...
## 2026-07-24：除子残数结构依赖检索

- 问题限定为有限阿贝尔群中避开指定二阶元的全部子序列积序列，关键词包括
  Kneser theorem、subsequence sums、finite abelian group、
  zero-sum sequence 与 long zero-free cyclic sequence。
- 检索入口为 arXiv、Numdam 和期刊 DOI 页面；纳入标准是直接陈述 Kneser 多集合定理
  或给出与偶阶循环群临界遗漏二阶元同型的原始研究。一般百科说明和二手讲义未纳入。
- 纳入 Grynkiewicz--Marchan--Ordaz 2009：Numdam 元数据、DOI
  10.5802/jtnb.689 与期刊 PDF 一致；已核对第 566--567 页 Example 2、
  第 567 页 Theorem C 及随后的 Kemperman 临界对讨论。
- Savchev--Chen 2006 的长零和自由循环序列论文用于检索交叉定位，但其目标是避开零
