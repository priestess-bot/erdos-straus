...
summary: 把状态合同中所有会产生 persistent successor 的输出统一到一个固定的全局相位注册表和七元自然数词典序势；terminal、analysis evidence、pending normalization 和 macro checkpoint 均不产生递归边。任何 E1--E4 已完成的动作只有在该规范势严格下降时才可标为 verified_edge，因此 RESET 回返、PRE/ABSORB 二环、standalone stutter 与无付款 phase reset 在合同层被永久排除。
topics:
  - proof-program
  - state-transition
...
> 对状态合同允许产生的任意 persistent successor edge，只要它通过 E1--E4，就存在**同一个、预先
> 固定且从状态本身可重算的**全局势，使该 edge 通过 E5；不满足这一势的动作不能成为 persistent
> successor，只能是 terminal、analysis evidence、internal checkpoint 或 rejection。

这比“给当前 allowlist 中的边逐个找一个下降量”更强，因为 recursive eligibility 的判据是合同层
的，不依赖手工边名单。
...
\eta_p(S)=
\begin{cases}
\nu_p(E-1),&\text{仅当 state 持有具名 immediate d=1 regeneration token},\\
...
## 6. 完整 admission theorem

一张产生 persistent target 的 receipt 在 E1--E4 后，只有以下三种 admission ticket：
...
state_id，并由 target verifier 从原始整数重算。
...
macro checkpoint。因此 persistent successor surface 被上述三种 ticket 穷尽。
