...
因此该 H4 clean \(q\)-bridge 是一个 verified decreasing macro。
...
\[
L_0=rac{\operatorname{lcm}(M_4,Q)}{M_4}.
\]
...
1. target `state_id` 只由规范整数 \((p,R',K',M)\)、scope/origin 和 adapter version 决定；
...
因此，若较早 priority action 未先闭合该 branch，则 H4 clean \(q\)-bridge 本身输出一条 verified decreasing E1–E5 macro。
...
这些 controls 只验证 arithmetic/serialization regression；它们**不替代** universal upstream H4 provenance 或 A11 的 universal stutter finite closure。
...
> 本 release 已在独立目录中运行新 verifier 与 6 个 focused tests。由于本会话没有对远端 GitHub 仓库的直接写权限，`kb.py validate/build`、CI 与真实 checkout 的最终集成验证仍应在应用补丁后的仓库 checkout 中执行。
...
\[
\boxed{
\textbf{方向 1：H4 clean q-bridge 的完整 E1–E5 相对闭包——完成。}
}
\]

后续研究不应继续把 H4 first-stutter 或 corrected q-support 当成 active arithmetic gap。除非独立审阅推翻某个上游 dependency，应把主攻方向转移到：
