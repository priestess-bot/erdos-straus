...
恒等提升没有从合成参数自动推出。后三行虽然没有 fixed-s 除子，但都被 A,C<=2、K
不设上限的 Type II AC 射线探针覆盖；其中 \(p=15601\) 还同时有
\(x=(15601+3)/4=3901=83\cdot47\) 的 gap-3 证书。这说明 fixed-s hard core
应先经过可变 K 的 AC 射线后置路由；若小 A,C 射线也失败，才进入 alternate、对偶容量
或跨状态良基势分析。
