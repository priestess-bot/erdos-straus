...
| proper-root \(m=3\) 双二次范数 slice | \(h=3u,a=3A\) 后得到精确互锁 gates \(A\mid3u^2-u+1\)、\(u\mid7A^2+A+1\)；root support 与核心模 \(24\) 强制 \(A\equiv3\pmod {24}\)、\((a,e-1)=3\)、\(k=3\kappa\equiv21\pmod {72}\)、\(k\ge93\)、\(W-\eta\ge13\)，fixed-\(\kappa\) fiber 满足 \(A\mid9(27\kappa^2+8\kappa+1)\)。whole \(d=(W-\eta,k)\) carrier 满足 \(d\mid p^2-p+1\) 且 \(\gcd(d,h(p^2+p+1)D)=1\)；其 \(d>1\) natural fan 还满足 \(C_d\ge8\)、\(s_d\le(p-8)/7\)。canonical \(q_\star\equiv7\pmod {12}\) 因而严格分为 root-supported、\(\Phi_6\)-fan 与 d-free quotient-only 三路；\(D\) 的 native quadratic form 判别式为 \(-11\) | `established`, `internal_review`；压缩 \(m=3\) 的新 adapter 输入，不给 terminal 或 E1--E5，QC1/TR1 仍开放 |
