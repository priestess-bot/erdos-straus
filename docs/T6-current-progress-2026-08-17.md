...
| proper-root \(m=3\) \(A_0=1\) automatic low-gap gate | 对 \(s\in\{3,7,11,23\}\)，\(\Delta_s=3s^2-s+1\) 的逐项分解表明正/负 root factor 的唯一可能 pair 是 \((s,q)=(3,5)\)。正根给出 direct Type II terminal；负根仅为既有 \(p-1,h+1,m+2\) overlap | `established`, `internal_review`；排空这个 fixed low-gap family，不构成 TR1 |
