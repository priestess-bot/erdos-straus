...
本节附加假设 $4K=pR+1$；没有这个假设只能得到
$E_j\equiv2L\pmod R$，不能把它称为目标终端。在此附加假设和上述二进条件下，
$E_j$ 是整数。由于 $2b$ 与 $R$ 互素，可在单位群中把
...
更强地，在没有必须保留的 source label 的终端层，每个合法 \((a,b,j)\) 都唯一归一为
一个给出相同 \((E,n)\) 的 \(j=1\) 互素 \(L\)-除子对。因此 \(j>1\) 只可作为 provenance
保留，不能扩张终端集合或容量维度；唯一可能落在目标指数盒外的是 \(v_2(E)=1\) 的一层
二进负边界。精确证明见
...
正反两个同余方向的指数存在性现可进一步规范化：每个方向只需检查其精确预算窗口中的
最大指数，而且两个方向的窗口若同时非空就必有一向满足高度条件。见
[双向广义二进窗口的规范最大指数选择引理](type-I-general-bidirectional-dyadic-window-selector.md)。
