...
\(R\le9999\) 的**全部通用周期**有限定理，不能再作为 (14) 的无反例证据外推。若先施加
真实核心可实现性，后续结果已把直接闭合范围推进到 \(R<100000\)，见下文链接。
...
进一步的 CRT--Dirichlet 构造已经证明：通用周期的完整坐标支撑可嵌入核心 \(K\)，当且
仅当支撑不含 3，或 \(R\equiv2\pmod3\)；而且可以令全部支撑赋值恰为 1。结合十万扫描，
(15) 是唯一 direct miss 且不满足该条件，所以 \(R<100000\) 的每个**真实核心支撑周期**
仍直接 radical 命中。精确量词见
[通用一层周期的核心可实现性与十万模数闭合](type-I-core-universal-cycle-realizability-and-100k-closure.md)。

所以新的开放对象不再是 (14)，而是以下更弱且更贴近原状态的析取：任意大 \(R\) 的实际
核心支撑周期是否总由 direct radical、三目标乘子桥命中，或由
