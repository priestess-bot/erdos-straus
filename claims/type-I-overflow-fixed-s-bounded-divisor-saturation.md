...
还有一个与最小素因子无关的直接分流。若 \(q\mid P\)、\(q\ge5\)，则

\[
L=\frac{P}{q}\le\frac{4B_p}{5}<B_p.
\]

只要 \(L\ge2A\)，又因 \(q<p\) 有

\[
s=\frac{4P+1}{p}<4L,\qquad K_L=L(p-q)>0.
\]

故任何满足 \(P>B_p\)、\(q\ge5\)、\(2A\le P/q\le B_p\) 的素因子都直接给出
fixed-s 恒等提升边。于是高乘积余项若没有这种边，除支撑饱和外只能使用
\(2,3\)-光滑的载体；这正是后续广义二进、Type II 或 q-进容量分支应处理的窄边界。
