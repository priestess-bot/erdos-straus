...
statement: 对五亿p减一桥遗漏的185个最短上半区源状态，119个直接有B=1的Type I正规形实现。其余66个从m不大于215的Type I盒完整生成全部偶上半区源状态后，65个可重选为B=1；唯一B=1遗漏为p=218482009，其所生成的上半区源状态均不存在B=1实现，但有最小B=3实现。184个B=1实现中有26个正规形缺口超过215，最大为597803。因此185点在该有限源状态生成窗口中均可重选为上半区B不大于3的Type I终端桥，但该窗口不界定重建后的B=1正规形缺口。
...
| 上半区 \(B\le3\) 闭合 | 185 |
| 已重建的 \(B=1\) 正规形缺口超过 215 | 26 |
| 已重建的 \(B=1\) 正规形最大缺口 | 597,803 |
...
说明从这个有限源状态窗口产生的上半区源不能全部立即收缩为 \(B=1\)。另一方面，26 个
\(B=1\) 重建缺口已经超过 215，故不能把该源状态窗口误读为 \(B=1\) 正规形的缺口界。
