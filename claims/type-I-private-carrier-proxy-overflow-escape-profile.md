...
后续的精确商群与关系格审计已经对 253 个冻结 F 状态的全部 1181 个支撑坐标计算

\[
\mu_q=\min_{z\in F}(|z_q|-\nu_q)_+.
\]

结果是所有坐标都满足 \(\mu_q=0\)，特别是本卡的 37 个私有代理坐标全部为零。因此
式 (12) 的 \(35+2\) 仍是正确的有限首见证分流，但这 37 个对象不能再解释为“真实
私有 \(q\) 缺陷”的正样本。

最小显式量词反例是 \(p=214729,R=43,K=151\cdot15287\)。确定性首见证在私有
\(q=15287\) 上代理过载，但完整目标纤维在原盒外的 Pareto 前沿为

\[
(0,5),(1,4),(2,3),(3,2),(4,1),(5,0),
\]

其中 \((5,0)\) 完全避开私有坐标。加入两个当前源块的逐坐标最大高度后，障碍仍然存在，
但变成联合前沿
...
而不是固定单坐标强制。完整统计和复现见
[冻结 F 状态的单坐标零缺陷与乐观扩展盒联合阻碍边界](type-I-f-selection-invariant-coordinate-and-optimistic-box-boundary.md)。
