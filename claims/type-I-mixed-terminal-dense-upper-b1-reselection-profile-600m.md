...
statement: 对500000000<p<=600000000的247个普通Type II p-1双尾遗漏，207个初始m<=215的B=1正规形已经给出偶上半区源n>=(p+1)/2。其余40个从同一有限盒完整生成上半区源状态后均有B=1正规形实现；其中3个重建缺口超过215，最大为18500439。故此连续有限区间的混合终端闭合可加强为“普通双尾或上半区B=1 Type I桥”，但不是m<=215的B=1正规形闭合。
...
| 经上半区源重选恢复 \(B=1\) | 40 |
| 上半区 \(B=1\) 遗漏 | 0 |
| 重选后 \(B=1\) 正规形缺口超过 215 | 3 |
| 重选后 \(B=1\) 正规形最大缺口 | 18,500,439 |
| 重选枚举的正规形 | 2,331 |
| 重选枚举的严格反向边 | 6,643 |
...
一阶关键自由度，但它也可能产生远大于源窗口的正规形缺口。它既不自动给出全称 \(B=1\) 界，
也不把第一个阶段的源选择变成固定规则。
