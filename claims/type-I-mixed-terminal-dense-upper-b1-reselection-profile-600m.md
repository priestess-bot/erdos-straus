...
statement: 对500000000<p<=600000000的247个普通Type II p-1双尾遗漏，均存在m<=215的Type I正规形和偶上半区源n>=(p+1)/2，使该源状态有B=1正规形实现。207个初始B=1记录已经在上半区；其余40个经同一有限盒内完整源重选后均恢复为上半区B=1。故此连续有限区间的混合终端闭合可加强为“普通双尾或上半区B=1 Type I桥”。
...
这与五亿范围内 \(p=218{,}482{,}009\) 的上半区 \(B=1\) 失败形成必要对照：源重选确实是
一阶关键自由度，但它既不自动给出全称 \(B=1\) 界，也不把第一个阶段的源选择变成固定规则。

## 范围

这是一个与先前五亿残余不重叠的连续区间压力测试。它支持“先选源、再选小 \(B\) 正规形”的
二层研究方向；没有证明任意核心素数都有相应终端桥，也没有给出跨区间可用的源状态构造。
