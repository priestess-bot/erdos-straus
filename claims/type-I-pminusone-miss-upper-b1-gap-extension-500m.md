...
title: 五亿 p减一遗漏经四缺口扩展的上半区 B 等于一闭合
statement: 对五亿p减一桥遗漏的185个点，m<=215时经完整上半区源重选已有184个具有B=1的Type I实现。唯一遗漏p=218482009在仅继续穷尽m=219,223,227,231后，于m=231出现三个上半区B=1桥，故185点均在m<=231的该有限模型中具有上半区B=1 Type I桥。该结果否定将短盒B=3回退解释为全局B=1障碍。
...
盒中留下唯一上半区 \(B=1\) 遗漏 \(p=218{,}482{,}009\)，并以 \(B=3\) 回退。该事实只说明
**固定短缺口盒**中不能要求 \(B=1\)，并不说明此素数在更大缺口处没有 \(B=1\) 桥。
...
| --- | ---: |
| \(p-1\) 桥遗漏 | 185 |
| \(m\le215\) 已经上半区 \(B=1\) 闭合 | 184 |
| 延伸窗口检查的 \(B=1\) 正规形 | 1 |
| 延伸窗口检查的严格反向边 | 3 |
| 于 \(m=231\) 释放的点 | 1 |
| \(m\le231\) 上半区 \(B=1\) 闭合 | 185 |
...
这个结果把当前下一步收紧为：不能把“一个 \(m\le215\) 的 \(B=1\) 失败”当作反驳全称
\(B=1\) 策略的证据。需要研究的是自适应缺口与源状态如何共同产生除子剩余命中，而不是固定
\(B\) 或固定缺口的单独菜单。

它仍只是一个 185 点、\(m\le231\) 的有限审计。它没有给出上界 \(m\le231\) 对任意核心素数
成立的理由，也没有证明原混合终端选择引理。
