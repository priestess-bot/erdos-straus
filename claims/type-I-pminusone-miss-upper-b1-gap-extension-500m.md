...
statement: 对五亿p减一桥遗漏的185个点，从m<=215的完整上半区源状态生成窗口已有184个具有B=1的Type I实现；按当前最短源优先选择，这些实现的最大缺口为597803。唯一遗漏p=218482009在仅继续直接穷尽m=219,223,227,231后，于m=231出现三个上半区B=1桥，故185点均具有上半区B=1 Type I桥。该结果否定将短盒B=3回退解释为全局B=1障碍，但不提供m<=231的统一缺口界。
...
缺口处没有 \(B=1\) 桥。其余 184 个状态虽有 \(B=1\) 实现，但按当前最短源优先选择，
这些重建正规形的最大缺口已达 \(597{,}803\)，并不落在这个短盒中。
...
| 按最短源优先选择时，前 184 个 \(B=1\) 实现的最大正规形缺口 | 597,803 |
| 延伸窗口检查的 \(B=1\) 正规形 | 1 |
| 延伸窗口检查的严格反向边 | 3 |
| 于 \(m=231\) 释放的点 | 1 |
