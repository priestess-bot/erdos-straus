...
statement: >-
  ...
  7 mod12 素因子 q_h；它必整除 u，因而是实际 root-capacity source-menu
  的 h-支撑输入，绝非 quotient-only。若 m>3，则 m=0 或 1 mod3；在
  ...
  最小素因子（或 whole D*）来自 actual maximal receipt 的真实 D occurrence，
  并严格属于 transverse provenance。于是 R4/R6 被全称分为 terminal、规范
...
式 (7) 把 (q_h) 绑定到 actual root-height receipt：
(u=(2r+1,(p^2+p+1)/3))。因此它属于既有 root-capacity
external-source menu 的真实输入类型，不是只由 (q_h\mid k) 得到的形式
quotient chart。
...
## 4. Actual transverse occurrence 对每个输入都存在
...
(D) 是 actual maximal decomposition (R-h=ED) 的 capacity-bounded factor。
故 (16) 不是裸整除性：它有 `D` occurrence、maximal receipt、source path 与
terminal priority 可以绑定。式 (15) 又表明该 occurrence 与 h-supported
root menu 严格不同。若 (q_T=2)，则它进入 dyadic transverse 子叶；不得调用只对
奇素数成立的 low-gap/negative-root menus。

这证明“无 transverse arithmetic carrier”的 family 为空。它没有证明
`TR1PhysicalTransitionV1` 存在：actual occurrence 仍缺确定 target、E3 owner、全域
E4 与 parent-to-final E5。
...
4. 若仍 miss，取 (16) 的 actual occurrence，送入 proposed
   `TR1PhysicalTransitionV1`。
...
R6 的 `k=3` 只把第一段 h-menu 置为空；它仍有 (14)--(16) 的 actual transverse
occurrence。一个最小失败 receipt 必须保存：
...
q_T occurrence id inside actual D
absence of any accepted TR1 target/ticket
```

缺失的是 occurrence 之后的数学边，不是 carrier existence。准确状态为：

```text
H_SUPPORTED_CARRIER_PARTITION = ESTABLISHED
NO_TRANSVERSE_ARITHMETIC_CARRIER = EMPTY
...
建立；R4/R6 必须重新证明自己的 actual path。其二，若未来 TR1 target support
(M\le B_p)，其 T5 票据必须是不可逆的 `CHARGED -> ABSORB` protocol drop；只有
(M>B_p) 且真实 parent-to-final CHARGED rank 严降时才可留在 CHARGED。二者都不能从
局部 checkpoint 的数值下降推断。
...
(D_*>1) occurrence 与稳定 residual codes。fixture 只是 typed partition control，
