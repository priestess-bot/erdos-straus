...
后续第二层 \(D\)-only 分析已经找到一个真正非自然 E4 正例：
\((p,R,K,E,n,\Delta)=(433,15,1624,16,432,2916)\)。但完整分类同时证明，第二层
marked fiber 一旦非空，\(\Delta\mid n^2\) 时已是另一张中心 Type I，反之已是直接
Type II；同一状态的外层关系 \(E=n=406\) 更只有自然候选。故“非自然提升是否存在”
的答案是肯定的，但这类显式非空第二层不会产生新的递归状态类型。见
[广义二进关系点的第二层 D-only 终端分类](type-I-generalized-dyadic-second-layer-d-only-terminal-classification.md)。
