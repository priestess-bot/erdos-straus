...
statement: 设同一核心素数 p 的两个不同线性源状态均落入 F 型低复杂度分支，且各自的稳定子群商为偶循环群；除一个奇素数 q_i 外的所有奇素因子都落在稳定子群中，q_iT_i 生成商群。若两个状态的活跃奇素数相同为 q，则 q 的最低共同指数整除 |R-R'|/4；并且 q 同时整除两个状态各自某个坐标块对应的 p-t，故 q 整除两个被选坐标标签之差。二进方向 2T_i 是否平凡必须另行处理。这是跨状态排除重复活跃方向的必要条件，不是全称选择器。
...
且除一个奇素数 \(q_i\mid K_i\) 外的所有奇素数在该商群中平凡，
\(q_iT_i\) 生成商群。记
...
这里不把二进残数 \(2T_i\) 隐含进“单一活跃”假设；若
\(2\in\mathcal H_{R_i}(K_i)\)，它可能在商群中非平凡，需使用
[二进块平衡约束](type-I-linear-dyadic-block-balance-obstruction.md)单独分类。
...
\(v_q(\lvert R_1-R_2\rvert/4)<\min(e_1,e_2)\)，则它们不可能共享同一个
奇素因子方向。若另外还知道 \(2T_i=T_i\)（且 \(2\in\mathcal H_{R_i}(K_i)\)），
则二进块平衡卡进一步排除该状态属于 F 型。
...
该结论只给出必要条件：它不排除不同状态使用不同活跃素因子的 F 障碍，也不处理二进方向
或多个奇素因子同时非平凡的情形。因此它推进了原三层重选猜想的跨状态部分，但尚未证明
该猜想。
