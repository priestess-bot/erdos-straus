...
同一个因子条件还允许更近的终端化。保持 (4) 的同一 \((A,B,C,R,K)\)，改取

\[
E=4,\qquad n=p-1.
\]

则 \(4K-E=3(p-1)\)，而 \(4\mid(p-1)^2/4\)。所以这也是一张上半区偶桥，且其源距离仅为一；
详见[九条固定 \(p-1\) 射线](type-I-fixed-universal-pminusone-b1-rays.md)。本卡的 \(E=2q\)
构造仍有独立价值：它直接从因子 \(q\) 把奇典范外源 \(N\) 终端化，并保留式 (13) 的显式源首分母。
