...
上式第二分支并非只由有限扫描支持。若 \((p+1)/2\) 有
\(q\equiv3\pmod4\) 的素因子，则[来自 \(p+1\) 的 \(B=1\) 上半区桥](type-I-p-plus-one-b1-upper-bridge.md)
显式构造 \(A=B=1\)、偶桥 \(E=((p+1)/q)^2\) 和源
\[
n=p-\left(\frac{p+1}{q}-1\right)>\frac{p+1}{2}.
\]
结合[(p+1)/2 的密度一筛界](p-plus-one-density-one-certificate.md)，这已证明第二分支本身覆盖
相对密度一的核心素数。其补集仍可能无限，故这条正面结果缩小的是最终逐点选择问题，而非完成它。
