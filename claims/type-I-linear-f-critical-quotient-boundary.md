...
$$
\mathcal H/T\cong C_{2m}.
$$

并假设只有一个素因子 (q_0\mid K) 在商群中非平凡，且 (q_0T) 生成整个商群。记
(e=v_{q_0}(K))，则

$$
\mathcal A/T
=\{1,q_0T,(q_0T)^2,\ldots,(q_0T)^e\}.
$$
...
特别地，(e=m-1) 是等号临界，(e\le m-2) 留下严格 F 型缺口，而
(e\ge m) 立即命中目标。这把四阶情形的“指数一”结论推广为任意偶循环商的
单方向预算判据。
...
在单一活跃素因子的循环商情形，每个除子在商群中的残数都是 (q_0T) 的幂，且
指数范围正好为 (0\le j\le e)，所以得到

$$
\mathcal A/T=\{(q_0T)^j:0\le j\le e\}.
$$

商群中的 (-1) 是 ((q_0T)^m)。因此它属于该除子谱当且仅当 (e\ge m)；若
(e<m)，谱大小为 (e+1)，半密度缺口就是 (m-(e+1))。
