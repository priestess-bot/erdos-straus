...
title: 两个局部余核的重复高度容量一代理面与短证书选择器
statement: 对先前遗留的(p,t)=(99151369,27337)、(487572409,106017)，历史代理把完整线性源谱中唯一重q块的一层当前高度再次加入原K指数盒，得到两例最低成本12、8个向量、4个反演对和4个overflow pattern；随后有限差值菜单给出11个经独立验证的Type II命中缺口并覆盖全部pattern，两例也各有一张逐项验证的同缺口Type I/II正规形。当前块饱和恒等式表明，这一“容量一”不是与真实分母缺陷相容的新容量，而是重复使用已包含在K中的高度；因此代理最小面只保留为候选生成实验。实际Type I/II证书仍有效，但本卡不推出overflow注入、真实q迁移、递降或全称选择器。
...
作解析证明。历史模型据此把重坐标指数界再放宽一层；旧卡中的“禁止重坐标溢出”则是
\(H=0\) 的边界实验。对
...
特别地，两例的重复高度容量一代理最低价都是 12。旧的 \(12,15\) 保留为 \(H=0\)
历史边界。后续当前块饱和定理说明：奇 \(q\) 上这唯一的一层已经包含在 \(K\) 的指数
预算中，而 \(e_q\) 是用尽该预算后的分母缺陷。因此 \(H=1\) 不是“实际下一最小面”，
也不存在尚待证明的当前块注入；真实桥必须从 \(K\) 外构造新资源或合法新状态。
...
所以重复高度代理最小面不能由共享和式或面内直接重分配闭合。这里的 13884 是三类菜单大小
之和；各菜单分别去重，不声称三类之间互不重叠。

## 过载坐标的差值候选选择器

把既有三通道高度作为历史候选生成阈值，两例的代理向量分别为
...
| \(p\) | overflow pattern | 严格超过历史代理阈值的坐标 | 可选命中缺口 |
|---:|:---|:---|:---|
...
注意第二个 \(99151369\) pattern 按历史代理阈值必须从坐标 \(227\) 生成候选；这只是有限
菜单的确定性规则，不能再解释为坐标 \(227\) 发生了真实容量过载。
...
相同缺口界内闭合，也不把候选差升级为实际迁移。后续完整谱复核又为这两个状态分别找到
\(R'=11,23\) 的较小 \(R\)、原私有 \(q\)-free Type I 命中和显式偶终端，见
[跨状态支撑退出与共享两尾等价](type-I-linear-private-carrier-support-exit-marked-equivalence.md)；
但这仍是两个有限实例。一般代理过载剖面中的 37 个私有状态只有 35 个存在较小 \(R\)
命中，另两个必须转入 Type II，见
[私有载体代理过载的有限分流](type-I-private-carrier-proxy-overflow-escape-profile.md)。
因此全称任务仍是：把选择不变的带符号分母缺陷映到另一 Type I 命中、独立 Type II 证书
或改变标记/支撑且严格可提升的递降。
