...
statement: 若线性状态的正规化因子满足 K=gamma*L，定义 D_R(X)=A_R(X)A_R(X)^(-1)，则有精确恒等式 D_R(K)=D_R(gamma)D_R(L)，且一般 B 目标 -1 命中当且仅当 D_R(L) 与 {-x^(-1):x属于D_R(gamma)} 相交；若该所需拉回集合在 H_L 中与 D_R(L) 的大小之和超过 |H_L|，则由鸽巢原理强制命中。对四个真实对抗核心中差集缺额不超过100的全部六个F型状态及十个有向源，逐项复核该恒等式和对齐判据；十个对齐交集均为空，且只有一个方向达到鸽巢等号边界。最大差集覆盖状态 (p,R)=(26034649,375) 的两种方向分别给出 (|D_gamma|,|D_L|)=(15,51) 与 (41,33)，乘积均覆盖192/200个生成子群元素，目标374仍缺失。该结果是块级有限负边界，不证明跨源选择器反例或定理。
...
\[
\boxed{
|\mathcal D_R(L)|+|T_\gamma\cap H_L|>|H_L|,
}
\tag{3}
\]

则 \(\mathcal D_R(L)\cap T_\gamma\ne\varnothing\)，从而命中目标。F 型状态必须满足
...
这个不等式是块级半密度条件的定向版本：它只数当前一块子群中真正需要的目标拉回类，
比比较整个 \(\mathcal H_R(K)\) 更精细。
...
| \(p\) | \(R\) | \(|\mathcal H_R(K)|\) | \(|\mathcal D_R(K)|\) | 缺额 | 各有向源的 \((|D_\gamma|,|D_L|)\) | 最小鸽巢余量 |
| ---: | ---: | ---: | ---: | ---: | --- |
| 26,034,649 | 375 | 200 | 192 | 8 | (15,51), (41,33) | 49 |
| 57,399,241 | 59 | 58 | 39 | 19 | (9,5) | 44 |
| 57,399,241 | 83 | 82 | 61 | 21 | (27,9), (9,27) | 46 |
| 57,399,241 | 455 | 288 | 246 | 42 | (54,5) | 0 |
| 57,399,241 | 155 | 120 | 59 | 61 | (3,33), (33,3) | 11 |
| 283,319,689 | 131 | 130 | 63 | 67 | (3,27), (27,3) | 100 |
...
在 \((p,R)=(57{,}399{,}241,455)\) 的方向 \((a,s)=(150,841)\) 中，
\(|H_L|=6\)、\(|D_L|=5\)，而 \(T_\gamma\cap H_L=\{391\}\)。因此 (4) 取等号，
且 391 正是右块唯一没有覆盖的所需类；这是鸽巢判据的尖锐等号边界。
