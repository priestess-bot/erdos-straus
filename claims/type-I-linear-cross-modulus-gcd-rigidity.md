...
更强地，令“层坐标数”表示该层中心化指数向量中的非零坐标数。九个混合命中里，
八个可用一个 \(P_R\) 坐标配合共享层达成目标，但

\[
p=3{,}942{,}409,\qquad R=10{,}951 \tag{9}
\]

是精确反例：其

\[
S_R=2^6\cdot5\cdot7\cdot11,\qquad
P_R=43\cdot61\cdot167
\]

的任何目标见证都至少使用两个 \(P_R\) 坐标；最小总坐标数为五。一个最小见证的共享、
独有坐标（按上面的素数顺序）分别为

\[
(-4,1,0,-1),\qquad(1,-1,0), \tag{10}
\]

即

\[
2^{-4}5\,11^{-1}\cdot43\,61^{-1}\equiv-1\pmod {10{,}951}.
\]

因此“共享指数层加一个独有素因子坐标必释放目标”的加强选择器已被这张完整线性谱反驳。
它不排除允许多个独有坐标的混合积集机制。
