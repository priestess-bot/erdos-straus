...
statement: 对核心素数 p 的线性源 p=a+s+asR，若 q>1 满足 q|s、q=1 mod a 且 R'=qR+(q-1)/a=3 mod4，则 (a,s/q,R') 是同一 p 的线性源，满足 aR'+1=q(aR+1)、q|K'；该边严格减小 s，故使源分母 p-s 严格增大而非递降。与固定 s 因子转移的前向边合并后，七个完整压力谱共有 719 条边，但仅 39/490 个状态可达目标谱命中，460 个失败状态中仅 9 个可达。即使允许反向走所有已验证边并加入全部可行的 a/s 坐标交换，156 个连通分量中仍有 135 个没有目标命中。因此这些局部线性源操作不能构成通用终端选择器。
...
即使只将上表 719 条**已验证**边视为可逆，也仍不能消除这一障碍：其无向化图分成
\(222\) 个连通分量，只有 \(27\) 个含目标命中；其余 \(195\) 个无目标分量含有
\(329\) 个状态。每个压力点都至少有一个这样的分量。因此，在这两个已明确构造的
转移的反演范围内，从这些分量出发仍不可能抵达目标谱。

这里没有声称枚举了任意反向因子重分配；结论只适用于已审计的两类边及其形式逆。
...
\[
(a,s,R)\longleftrightarrow(s,a,R) \tag{14}
\]

是同一 \(p\) 的两个合法有向线性源状态：它保持 \(R\) 和 \(K\)，只改变作为源移位的
奇坐标。这个交换是一个精确的可逆局部操作，不能在状态图审计中遗漏。

在七个完整谱中有 \(130\) 条不同的非平凡交换边。将它们加入上面的无向化图后，连通分量
数从 \(222\) 降为 \(156\)，含目标命中的状态数只从 \(161\) 增为 \(166\)；仍有
\(135\) 个无目标分量、共 \(324\) 个状态。故即使补全这种坐标对称性，已审计的局部
线性源操作依然不能跨越大多数无目标分量。
