...
statement: 对核心素数 p 的线性源 p=a+s+asR，若 q>1 满足 q|s、q=1 mod a 且 R'=qR+(q-1)/a=3 mod4，则 (a,s/q,R') 是同一 p 的线性源，满足 aR'+1=q(aR+1)、q|K'；该边严格减小 s，故使源分母 p-s 严格增大而非递降。与固定 s 因子转移的前向边合并后，七个完整压力谱共有 719 条边，但仅 39/490 个状态可达目标谱命中，460 个失败状态中仅 9 个可达。即使允许反向走所有已验证边，222 个连通分量中仍有 195 个没有目标命中。因此这两类局部因子转移不能构成通用终端选择器。
