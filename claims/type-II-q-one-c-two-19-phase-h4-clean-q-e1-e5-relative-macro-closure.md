...
statement: >-
  ...
  (0,c_q)<(0,p-1)。因此在上述、已外部验证的 upstream actual-H4 receipt 与
  priority-prefix miss 前提下，H4 clean q-bridge 已闭合为 phase-local decreasing macro；
  局部 arithmetic control 只验证相对计算，不能登记为递归边。若较早 priority
...
通用 verifier 因而区分两种输入：带
`verified_actual_h4_provenance` 和 `verified_priority_prefix_miss` 状态的 receipt 可以
产生 `candidate_transition` 的 phase-local macro；仅用于 p=73、241 回归的 control receipt 只能产生
`control_only`，不得递归入队。
...
若版本化的较早 priority action 已命中一个 verified terminal/edge，则 H4 分支已提前关闭；否则 priority-prefix miss 后，canonical H4 clean-q macro 满足 E1--E5，输出 phase-local `candidate_transition`。在全局 selector 与跨 phase 良基势接纳前，它不得标为 `verified_edge`。
...
## 12. 与现有 parent-anchored 宏的关系

本卡是 clean \(q\) 后缀的相对闭包：它消费 upstream actual-H4 receipt，而不重放
\(P\Rightarrow H_4\) 的完整 persistent lineage。主仓库的
[H4 \(a=1\) parent-anchored checkpoint 收缩宏](type-II-q-one-c-two-19-phase-h4-a-one-q-bridge-atomic-macro-checkpoint-contraction.md)
在其适用域内重放更强的 source/typed target 语义；两者不互相替代。本卡的价值是固定
修正后的 \(M_q=\operatorname{lcm}(M_4,Q_x,Q_y)\) 接口、相对 E1--E5 推导和可独立
调用的局部 verifier。
