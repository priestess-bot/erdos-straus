...
statement: 若目标纤维到原指数盒的内禀 L1 距离 Psi_0 等于 1，令 D 为全部最短见证可能溢出的坐标集，则 D 必包含于每一组投影盒阻碍 J；反向包含一般不成立。对冻结平方终端链中的 55 个 Psi_0=1 状态，完整一层壳共有 140 条正向最短见证及其 140 条符号反射，D 的大小为 1 至 5；55 个最小投影阻碍都唯一，但只有 11 个满足 D=J。D 与规范双活跃 Fourier 支撑相交 49/55，另 6 个完全不相交，故最近纤维不能普遍压到活跃方向。把最短见证直接剪去一层后，既有广义 2^j 偶终端判据在全部 280 条带符号见证上零命中；从缺陷素数 q=8h-1 提取 Type II K=2 相邻桥候选也在 6 个合法候选上零命中。进一步把每条见证定向为唯一负缺陷并完整参数化固定 B 的精确 gcd 约分端点，在必要大小界内共得 881472 个互素端点，任意 Type II 命中仍为零。这些零命中都只是冻结样本中指定映射的负边界，不排除其它二进终端、Type I/II 证书或支撑逃逸规则。对所有合法内部因子 M|K 使用新的 R 坐标残数拉回，完整菜单在同一 55 态中直接终端 37 态、遗漏 18 态；只取缺陷坐标仅终端 8 态。一层见证还具有显式外部分母迁移恒等式，它严格降低形式对的良基势，但 K 不能吸收的剩余因子尚未被实现为合法 F/G 状态、解提升或原猜想的递降。
...
前面的三个零命中结果只针对“剪掉缺陷层后二进终端”“缺陷素数相邻桥”和“固定负缺陷
约分端点”三种指定映射，并不排除重新选择首分母缺口。由 \(4K=pR+1\)，对任意合法
内部因子
...
补上 \(d\mid x_M^2\) 后，式 (46) 分别是精确 Type I/II 终端判据；第二式可用互补
除子规范到 \(d\le x_M\)。该拉回对复合 \(M\) 同样成立。

对本卡冻结的 55 态完整枚举全部合法 \(M\mid K\)，共检查 1102 个缺口和 119922 个
平方除子残数。62 个缺口命中，状态层面为

\[
\boxed{37\text{ hit},\qquad18\text{ miss}.}
\tag{47}
\]

37 张规范首证书分为 13 张 Type I 与 24 张 Type II。若只取本卡的 120 个缺陷坐标，
只有 60 个是合法缺口，最终只有 9 个坐标、8 个状态命中；六个 \(q=7\) 缺陷坐标也
全部避开已有的三个 \(R\bmod7\) 证书类。最小的完整内部菜单遗漏是

\[
(p,R,K)=(37793809,19,7\cdot53\cdot483883),
\]

其全部合法内部缺口 \(7,371,483883,25645799\) 均失败。完整定理、复现剖面和量词边界
见 [K 内部缺口的 R 坐标残数拉回](internal-support-gap-residue-pullback.md)。

式 (47) 是新增的直接终端菜单，不改变此前三个指定映射的零命中；它也不是后继状态。
因此余下 18 态仍需周期外终端、跨图表命中或满足 E1--E5 的合法递降。
