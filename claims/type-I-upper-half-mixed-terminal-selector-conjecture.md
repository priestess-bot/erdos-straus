...
证明 (1) 的本质不是增加扫描范围，而是构造一个跨正规形、跨源状态的选择原理：它必须
从 \(p\) 的实际因子状态产生一个菜单外的 \((s,E)\)，再通过
[源状态实现判据](type-I-normal-source-state-realization.md)强制一张正规形的小侧对；或者直接
强制普通双尾。只跟踪字符核、总积同余或固定有限候选盒均不足以完成这一步；这些信息不会控制
因子指数、源状态和跨正规形的重选。
