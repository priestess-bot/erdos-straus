...
更强地，把已穷尽的 \(B=1\) 审计、源状态重选和少量缺口延伸作精确分流后，全部 1,717 点
都有上半区 \(B=1\) Type I 桥；所选正规形最大缺口为 \(5963\)，见
...
以源状态的 \(B=1\) 实现进一步审计时，247 个点中 207 个初始 \(m\le215\) 的 \(B=1\) 记录
已经在上半区；其余 40 个均可由同一盒生成的上半区源重选恢复为 \(B=1\)，见
[六亿上半区 B等于一重选剖面](type-I-mixed-terminal-dense-upper-b1-reselection-profile-600m.md)。
将重选准则改为正规形缺口优先后，40 个重选点的最大缺口为 \(87\)，并与直接记录合成
\(m\le131\) 的全区间闭合。这为“先重选源、再取小 \(B\)”提供独立区间证据，但不能合并为
统一 \(B=1\) 定理，更不能从一个有限区间的上界推出全称缺口界。
...
但这个 \(B=3\) 例外也只属于短源状态窗口：仅将直接正规形搜索延伸到 \(m=231\)，就恢复三个
上半区 \(B=1\) 状态，见
[p减一遗漏 B等于一缺口扩展](type-I-pminusone-miss-upper-b1-gap-extension-500m.md)。不过前 184 个
\(B=1\) 重建的 \(597{,}803\) 最大缺口来自最短源优先的选择，并非最小缺口下界；目标级重选
已在整个 1,717 点样本中将所选最大缺口降到 \(5963\)。故 185 点仍不能仅从该卡推出
\(m\le231\) 闭合，但“高缺口是内在需求”同样没有证据。
所以“固定源上的低指数溢出”已经失败为全局解释，但当前数据**尚不能**反驳允许自适应缺口的
全称 \(B=1\) 策略；源状态、正规形和缺口必须作为耦合变量选择。
...
强制普通双尾。这里第一层不能固定桥菜单、源距离或预先选定源状态；第二层必须同时控制
源状态生成与其 \(B=1\) 或一般 \(B\) 正规形实现。当前数据表明某个最短源选择不控制实现缺口，
但目标级三变量重选可大幅降低有限样本中的最大缺口。下一步须分析可变 \(m\) 如何与 \((s,E)\)
及因子对共同强制除子剩余命中。只跟踪字符核、总积同余或固定有限候选盒均不足以完成这一步；
这些信息不会控制因子指数、源状态和跨正规形的重选。

当前有限证据还支持一条更强、也更容易被反例推翻的优先子路线：直接要求目标第一层为普通
除子，即 \(B=1\)，见[自适应上半区 B等于一选择猜想](type-I-adaptive-upper-b1-terminal-selector-conjecture.md)。
它不是原目标的等价改写；若失败，应退回本卡允许一般 \(B\) 的选择器，而不能据此否定
Erdős--Straus 猜想。
