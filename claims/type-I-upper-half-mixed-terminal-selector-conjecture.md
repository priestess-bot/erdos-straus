...
但这个 \(B=3\) 例外也只属于短盒：仅将其缺口延伸到 \(m=231\)，就恢复三个上半区 \(B=1\)
状态，故 185 点均在 \(m\le231\) 闭合，见
[p减一遗漏 B等于一缺口扩展](type-I-pminusone-miss-upper-b1-gap-extension-500m.md)。所以“固定源上
的低指数溢出”已经失败为全局解释，但当前数据**尚不能**反驳允许自适应缺口的全称 \(B=1\)
策略；源状态、正规形和缺口必须作为耦合变量选择。
...
强制普通双尾。这里第一层不能固定桥菜单、源距离或预先选定源状态；第二层至少不能在**固定缺口盒**
中全局固定为 \(B=1\) 或固定指数溢出阈值。下一步须分析可变 \(m\) 如何与 \((s,E)\) 共同
强制除子剩余命中。只跟踪字符核、总积同余或固定有限候选盒均不足以完成这一步；
