...
class State:
    ...
    def __post_init__(self):
        ...
        if any(value < 0 for value in (self.q, self.eta, self.a, self.R, self.m, self.r_eps, self.reset_M)):
            raise ValueError("local rank fields must be nonnegative")
    ...
    def rank(self):
        rank = (self.rho, int(self.major), int(self.protocol), *self.local())
        assert len(rank) == 7 and all(value >= 0 for value in rank)
        return rank
...
def verify():
    ...
    # Every local component used by Pi_T5 is genuinely N-valued.
    invalid = (
        lambda: charged(p, eta=-1),
        lambda: State(p, Major.TYPEI, Protocol.PRE, p=p, a=-1),
        lambda: State(p, Major.TYPEI, Protocol.ABSORB, p=p, R=-1),
        lambda: State(p, Major.TYPEI, Protocol.RESET, p=p, reset_M=-1),
    )
    for build in invalid:
        try:
            ...
            raise AssertionError("negative local rank was accepted")
