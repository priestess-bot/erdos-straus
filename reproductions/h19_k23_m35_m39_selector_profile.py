#!/usr/bin/env python3
"""Profile m=39 support after the H19-k23 m=35 selector boundary."""
...
DEFAULT_OUTPUT = ROOT / "reproductions" / "h19-k23-m35-m39-selector-profile-262144.json"
BRANCHES = ROOT / "reproductions" / "mixed_factor_h19_uniform_affine_boundary.py"


def load_branches():
    spec = importlib.util.spec_from_file_location("h19_k23_m39_branches", BRANCHES)
...
def smooth_base_divisors(factors: dict[int, int]) -> list[int]:
    """All 2,5-smooth divisors of 10^2*u^2, including u's base-prime powers."""
    return [
        2**two_power * 5**five_power
        for two_power in range(2 + 2 * factors.get(2, 0) + 1)
        for five_power in range(2 + 2 * factors.get(5, 0) + 1)
    ]
...
def selector_divisor(u: int, support_size: int) -> int | None:
    """Find an m=39 divisor with zero, one, or two genuinely new primes."""
    ...
    candidates = [
        ...
        if base_divisor * product <= 10 * u and (base_divisor * product) % 39 == target
    ]
    return min(candidates) if candidates else None


def verify_witness(prime: int, divisor: int) -> None:
    normalized = family.verify_normal_form(prime, 39, divisor)
    if normalized["q"] != 10:
        raise AssertionError("m=39 did not recover q=10")


def run_profile(payload: dict[str, object]) -> dict[str, object]:
    """Classify m=39 exits after stored m=31 and m=35 selector misses."""
    ...
    for branch in branches:
        ...
        if coefficient % 40 or (int(form["constant"]) - 1) % 40:
            raise AssertionError("m=39 is not universally available on the branch")
    ...
    for record in payload["records"]:
        if (
            ...
            or int(record["tail_witness"]["gap"]) in {31, 35}
        ):
            continue
        prime = int(record["prime"])
        if prime % coefficient not in constants:
            raise AssertionError("m=35 miss is outside the residual branch set")
        u = (prime + 39) // 40
        ...
        if tail_gap == 39:
            if fixed is not None:
                ...
                counts["smooth_base_m39_hit"] += 1
            elif one_new is not None:
                verify_witness(prime, one_new)
                counts["one_new_prime_m39_hit"] += 1
            elif two_new is not None:
                verify_witness(prime, two_new)
                counts["two_new_prime_m39_hit"] += 1
            else:
                raise AssertionError("m=39 tail has no support-two selector")
        else:
            if fixed is not None or one_new is not None or two_new is not None:
                raise AssertionError("bounded m=39 selector contradicted the stored first tail gap")
            counts["m39_miss"] += 1
            later_tail_gaps[tail_gap] += 1
    total = sum(counts.values())
    return {
        "arithmetic": (
            "exact branch congruences for 40|p-1, complete 2,5-smooth-base "
            "extension plus one- and two-new-prime-power divisor enumeration, and "
            "exact q=10 square-root-completion verification for every constructed witness"
        ),
        "scope_note": (
            "A finite support profile after stored m=35 misses. It does not prove "
            "a universal support bound for the m=39 selector."
        ),
        "input_parameter_limit_exclusive": payload["input_parameter_limit_exclusive"],
        "m35_miss_record_count": total,
        "all_branches_have_40_dividing_p_minus_one": True,
        "smooth_base_m39_selector_hit_count": counts["smooth_base_m39_hit"],
        "one_new_prime_m39_selector_hit_count": counts["one_new_prime_m39_hit"],
        "two_new_prime_m39_selector_hit_count": counts["two_new_prime_m39_hit"],
        "m39_selector_miss_count": counts["m39_miss"],
