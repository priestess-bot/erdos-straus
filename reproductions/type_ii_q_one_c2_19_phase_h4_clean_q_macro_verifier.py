...
ADAPTER_VERSION = "h4_clean_q_e1_e5_relative_macro_v2"
INPUT_SCHEMA = "h4-clean-q-macro-input/v2"
OUTPUT_SCHEMA = "h4-clean-q-macro-receipt/v2"
...
UPSTREAM_PROVENANCE_ACTUAL = "verified_actual_h4_provenance"
UPSTREAM_PROVENANCE_CONTROL = "local_arithmetic_control_only"
PRIORITY_PREMISE_ACTUAL = "verified_priority_prefix_miss"
PRIORITY_PREMISE_CONTROL = "local_control_only"
...
def verify_h4_macro(inp: H4Input) -> dict[str, Any]:
    ...
    require(upstream_claim == EXPECTED_UPSTREAM_CLAIM,
            "upstream H4 claim_id is not the expected actual-H4 provenance claim")

    upstream_status = str(inp.upstream_h4.get("validation_status", ""))
    priority_status = str(inp.priority_prefix.get("validation_status", ""))
    if upstream_status == UPSTREAM_PROVENANCE_ACTUAL:
        require(priority_status == PRIORITY_PREMISE_ACTUAL,
                "a verified upstream H4 receipt requires a verified priority-prefix miss")
        verification_scope = "relative_macro_with_verified_external_premises"
        phase_local_macro_eligible = True
    elif upstream_status == UPSTREAM_PROVENANCE_CONTROL:
        require(priority_status == PRIORITY_PREMISE_CONTROL,
                "a local control must use the local-control priority status")
        verification_scope = "local_arithmetic_control_only"
        phase_local_macro_eligible = False
    else:
        raise VerificationError(
            "upstream H4 receipt must declare verified_actual_h4_provenance "
            "or local_arithmetic_control_only"
        )
    ...
    # State identity deliberately excludes lazy dispatch fields and derived
    # classifier labels.  Those fields may change only through normalization.
    target_identity = {
        "state_type": "linear_absorbed_support_v1",
        "state_version": "linear_absorbed_support_v1",
        "p": p,
        "R": r_target,
        "absorbed_support": m_target,
        "source_tree_scope": "charged_history_only",
    }
    target_state_id = digest("state", target_identity)
    target_core = {
        **target_identity,
        "K": k_target,
        "capacity": c_target,
    ...
    require(target_rank < source_rank, "phase-local H4 macro rank does not strictly decrease")
    ...
    return {
        ...
        "premise_verification": {
            "upstream_h4": upstream_status,
            "priority_prefix": priority_status,
            "phase_local_macro_eligible": phase_local_macro_eligible,
        ...
        "selector_status": "candidate_transition" if phase_local_macro_eligible else "control_only",
        "phase_local_macro_eligible": phase_local_macro_eligible,
        "recursive_edge_eligible": False,
        "edge_id": edge_id,
    }
...
def make_control_input(p: int, peeled_part: int, name: str) -> H4Input:
    ...
    return H4Input(
        ...
        priority_prefix={
            ...
            "validation_status": PRIORITY_PREMISE_CONTROL,
        },
        upstream_h4={
            "claim_id": EXPECTED_UPSTREAM_CLAIM,
            "receipt_id": f"local-arithmetic-control:{name}",
            "validation_status": UPSTREAM_PROVENANCE_CONTROL,
        },
    )
...
def verify_controls() -> list[dict[str, Any]]:
    ...
    for f in CONTROL_FIXTURES:
        ...
        out.append({
            ...
            "selector_status": receipt["selector_status"],
            "phase_local_macro_eligible": receipt["phase_local_macro_eligible"],
            "recursive_edge_eligible": receipt["recursive_edge_eligible"],
