...
\(\ell\mid R\) 时精确 \(\ell\)-扭量还强制 \(\ell^2\mid R\)，并以
...
## 2026-08-12：双 G 可严格避开至 gap 55

\(p=118801\) 是新的双 G 压力点：

\[
\frac{p+3}{4}=29701=7\cdot4243,
\qquad
\frac{3p+1}{4}=89101,
\]

所有出现的素因子均为 \(1\pmod3\)。逐一穷尽 gaps
\(3,7,\ldots,55\) 的完整平方除子谱后，Type I 与 Type II 目标均未命中；因此它严格
否定“固定 gap 扇到 55 足以关闭双 G”的候选。首个证书在 \(m=59\)：
\(x=29715\)、\(d=21\) 给出 Type II terminal，且经
\((A,B,C,K)=(1,1415,21,24)\) 同时产生
\(n=(p+59)/60=1981<p\) 的两尾标记递降。
...
## 2026-08-12：奇 \(h\) 补齐 gap-23 的二次非剩余出口

令 \(p=24h+1\)、\(x=(p+23)/4=6(h+1)\)。先前的 gap-23 自动扇只用
\(36\mid x^2\)，覆盖 \(p\bmod23\) 的九个二次非剩余类；两个遗漏类是
\(5,14\)。若 \(h\) 为奇数，则 \(12\mid x\)，故额外的强制除子
\(d=16,8\) 分别补齐

\[
p\equiv5\pmod {23},\qquad p\equiv14\pmod {23}.
\]

与既有九项表合并后，奇 \(h\) 且 \(p\) 为模 \(23\) 二次非剩余时，gap 23 必有
显式 Type II terminal。由于 \(24\mid p-1\)，每一张此类 Type II 证书还经互素
因子正规形给出 \(n=(p+23)/24=h+1<p\) 的标记两尾解和不读取目标解的 lift。

这为双 G 严格控制 \(p=2521\) 给出结构性解释而非扫描事实：\(h=105\) 为奇数、
\(p\equiv14\pmod {23}\)、\(d=8\)，所以它严格下降到 \(n=106\) 并恢复其 gap-23
terminal。仍未覆盖偶 \(h\) 或奇 \(h\) 的模-23 二次剩余支。
...
\((m,a)\)。更关键地，\(u\) 为奇数、\(m\) 奇时既有 \(D,Da\) 的 parity 和 \(m\)
偶时 \(m\mid a+3u\) 一起给出 \(a\) 始终为奇数。令
\(L=am\)、\(s=m-a\)、\(\mathcal B=L^2+Ls+s^2\)。故 \(a\ge3\)、\(L\ge9\)，
并且有精确包络分解
...
这里的 \(L\) 还给出完整的 q-primary 分流：\(L=1\) 只可能是
\((s,q)=(3,5)\) 或 \((7,13)\)，并精确回到既有的
\(p-1,h+1,m+2\) overlap；\(L>1\) 则同时避开 \(p\pm1\)、\(2p+1\)、
\(m\)、\(m+2\)、\(m-1\) 与 \(h^2-1\)，使全部 \(q\)-进高度落在
...
\(D\mid ph+1\)，故 terminal-first 可先检查完整菜单

\[
\mathcal M_{\rm raw}(p,h,D)
=\{Q\mid D:Q\equiv-1\pmod {4h}\}.
\]
...
这精确覆盖 \(A=1,K=h\) 且生成模数整除 \(D\) 的 raw-ray 切片；\(D_*\) 给出其中的
横向子菜单，它不是对所有 Type II 证书的分类。每个 menu hit 还不可能完全整除
\(D_C=(D,(p^2-1)/2)\mid h^2-1\)，所以它不能只消费 \(p\pm1\) C-side 容量，
而必须读取超过该部分的 actual receipt 信息。root-shape 控制
\((p,h,Q)=(10369,21,335=5\cdot67)\) 显示精确菜单必须保留合数生成模数。菜单仍可能
为空，也没有 identity lift，因此它只是把 actual receipt 接到一张新的 terminal-first
检查表，而非 global exit。详见
[根容量 stutter 回执除子的原生 Type II raw-ray 终端菜单](../claims/type-I-root-capacity-stutter-transverse-native-raw-type-II-menu.md)。
...
子群，当且仅当该尺度存在无限 \(Q\)-supported prime ray。由于 \((6k-1)^2\equiv k\pmod{4k-1}\)，
这个群门又等价于子群含 \(-1\)。特别地 \(k=3\) 对所有核心素数可选且群门恒开；但最小
\(e=17^7\) 仍强迫 \(p\equiv-11^{-1}\pmod{17^4}\)，所以实际 CRT 容量而非群门才是
普适选择器的障碍。双素数控制 \(p=3444913,k=6,e=5^3 7^6\) 实现了这张完整菜单的
非单幂部分。这个容量图仍不保证给定 \(p\) 落在某个 CRT 类，也不构成全称 exit。

同余强制的四尺度束 \(\{1,2,3,6\}\) 也不能代替变量选择器：对无穷多个
\(p\equiv1\pmod{14280=24\cdot5\cdot7\cdot17}\)，这四个尺度上的
\(Q\)-supported 完整平方因子菜单同时为空。这里 \(k=2\) 的群门恒关；其余三个尺度的
允许支撑分别为 \(\{5\},\{17\},\{5,7\}\)，而这些素数均不整除
